...
function ioEmail(email){
    let email = "alaasdxcsa@sdds.com";
    let capA = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/g;
    let res = capA.test(email);
    // Add your logic.
    return res;
}
...
*/
/http:\/\/\S+(\.png|\.jpg|\.gif)/g
function imagesSearcher(text){
    let arr = [];
    let text = "alaasdxcsa@sdds.com";
    let ima = \S+(\.png|\.jpg|\.jpeg)/g;
    let apr = ima.test(text);
    if (apr = true){
        arr.push(apr);
